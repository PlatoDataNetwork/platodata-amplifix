...
import { Loader2, FileText, ExternalLink, CheckCircle, XCircle, Rss } from "lucide-react";
...
const FeedSyncLogs = () => {
  ...
  // Fetch sync logs with feed names
  const { data: logs, isLoading } = useQuery({
    queryKey: ["feed-sync-logs", selectedFeed],
    queryFn: async () => {
      let query = supabase
        .from("feed_sync_logs")
        .select(`
  ...
  const successfulImports = logs?.filter(log => log.article_id !== null).length || 0;
  const skippedDuplicates = logs?.filter(log => log.article_id === null).length || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-bold text-primary">{totalLogs}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <p className="text-2xl font-bold text-muted-foreground">{skippedDuplicates}</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Showing the last 100 sync operations
              </CardDescription>
            </div>
